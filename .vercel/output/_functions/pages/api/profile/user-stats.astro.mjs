...
const GET = async ({ locals }) => {
  try {
    ...
    const statsPromise = Promise.all([
      ...
      supabase.from("user_printing_stats").select("*").eq("user_id", userId).single().then(({ data, error }) => {
      ...
      supabase.from("user_loan_stats").select("*").eq("user_id", userId).single().then(({ data, error }) => {
